...
| `docs/spec-incident-list-by-assignment-group.md#scope`; `crates/snow_daemon/src/rpc.rs` | Daemon method is routable and declared in `contract_info` | T2 | `RpcMethod`, dispatcher, `SUPPORTED_RPC_METHODS` | JSON-RPC consumer test executes the method and bridge contract lists it as supported | Daemon coder |
...
- `crates/snow_core/src/service/record.rs`, `crates/snow_core/src/facade.rs`,
  ...
  and output.
- `crates/snow_daemon/src/rpc.rs` for the daemon JSON-RPC operation. The
  daemon's MCP surface is **not** `crates/snow_daemon/src/mcp.rs` — that file is
  an orphan, declared by no `mod` statement and never compiled. The daemon hosts
